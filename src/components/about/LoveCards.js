...
function LoveCards() {
  return (
    <div className="flex flex-wrap justify-center gap-2 lg:gap-0 lg:justify-between">
      <LoveCardItem src={grass} text="sitting in the grass" />
      <LoveCardItem src={val} text="trying to hit plat" />
      <LoveCardItem src={cereal} text="eating cereal" />

      <LoveCardItem src={debussy} text="learning debussy's arabesques" />

      <LoveCardItem src={marina} text="looking for marina" />
      <LoveCardItem src={snow} text="waiting for it to snow" />
      <LoveCardItem src={out} text="reading OUT by Natsuo Kirino" />
    </div>
  );
}
